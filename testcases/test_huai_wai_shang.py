...
from common.read_yaml import new_report_title_huai_wai_shang_1, new_report_title_huai_wai_shang_2, new_report_title_huai_wai_shang_3, new_report_title_huai_wai_shang_4, new_report_title_huai_wai_shang_5
...
class TestAddReportHuaiWaiShang:
    ...
    @pytest.mark.smoke
    @allure.story("报告添加-外伤合并退变，增加外伤显示正常结论-踝")
    @allure.title("添加成功测试-{fangshe_bianhao}-{buweimingcheng}-{leibie}-{xibuwei}-{tree1}-{expect}")
    @pytest.mark.parametrize("fangshe_bianhao, buweimingcheng, leibie, xibuwei, tree1, expect", new_report_title_huai_wai_shang_5)
    # 多参数的参数化，这样写的话参数可以直接使用，但在parametrize与测试函数的形参中需要列出所有的参数，并且参数的顺序必须一致
    def test_add_report_huai_wai_shang6(self, fangshe_bianhao, buweimingcheng, leibie, xibuwei, tree1, expect, login_and_navigate_to_jiegouhua):
        page = login_and_navigate_to_jiegouhua
        page.page_add_report_huai_waishang_title3(fangshe_bianhao, buweimingcheng, leibie, xibuwei, tree1)
        with allure.step("查看报告结果"):
            info = page.page_get_form_info_content()
        with allure.step("开始断言"):
            assert expect in info