...
from common.read_yaml import new_report_bi_tian_title3, new_report_title_jian_1
...
class TestAddReportJianTuiBian:
    ...
    @allure.title("添加成功测试-{fangshe_bianhao}-{buweimingcheng}-{leibie}-{xibuwei}-{treename}-{suntree}-{ban_suizhong}-{expect}")
    @pytest.mark.parametrize("fangshe_bianhao, buweimingcheng, leibie, xibuwei, treename, suntree, ban_suizhong, expect", new_report_title_jian_1)
    # 多参数的参数化，这样写的话参数可以直接使用，但在parametrize与测试函数的形参中需要列出所有的参数，并且参数的顺序必须一致
    def test_add_report_jian1(self, fangshe_bianhao, buweimingcheng, leibie, treename, suntree, ban_suizhong, expect, login_and_navigate_to_jiegouhua):
        page = login_and_navigate_to_jiegouhua
        page.page_add_report_jian_tuibian_title1(fangshe_bianhao, buweimingcheng, leibie, treename, suntree, ban_suizhong)
        with allure.step("查看报告结果"):
            info = page.page_get_form_info_rule_title()
